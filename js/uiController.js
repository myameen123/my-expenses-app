  ...
  return {
    ...
    pblGetInput: function () {
      ...
      return {
        ...
        description: document.querySelector(prvDOMstrings.inputDescription)
          .value,
    ...
    // Parameters:
    //   - transaction (Object): Expense/Income instance - Transaction entered by the user.
    ...
    pblAddListItem: function (transcation, type) {
      ...
      if (type === "inc") {
        ...
        transactionListItem =
          '<div class="item clearfix" id="inc-%id%"><div class="item__description">%description%</div><div class="right clearfix"><div class="item__value">%value%</div><div class="item__delete"><button class="item__delete__btn"><i class="ion-ios-close"></i></button></div></div></div>';
      ...
      // if the type of transaction is an expense
        ...
        transactionListItem =
          '<div class="item clearfix" id="exp-%id%"><div class="item__description">%description%</div><div class="right clearfix"><div class="item__value">%value%</div><div class="item__percentage">-</div><div class="item__delete"><button class="item__delete__btn"><i class="ion-ios-close"></i></button></div></div></div>';
      ...
      newTransactionListItem = transactionListItem.replace(
        "%id%",
        transcation.id
      );
      newTransactionListItem = newTransactionListItem.replace(
        "%description%",
        timeDateObj.timeDateCustomFormat + transcation.description
      );
      newTransactionListItem = newTransactionListItem.replace(
        "%value%",
        rpJSframework.pblFormatNumberUsingSystemLocale(transcation.value)
      );
      ...
      document
        .querySelector(transListRootElemClass)
        .insertAdjacentHTML("beforeend", newTransactionListItem);
    },
    ...
    // Public method for displaying the percentage of each expense out of the total income
    // Parameters:
    //   - expensePercentageTotalIncomeArray (Array): Array instance - Array with the expense percentages out of the total income for each expense.
    pbldisplayPercentageOfExpenseOutOfTotalIncomeForAllExpenses: function (
      expensePercentageTotalIncomeArray
    ) {
      var expensePercentageNodes;

      // select all the elements that have the item__percentage class
      expensePercentageNodes = document.querySelectorAll(
        prvDOMstrings.expensePercentageLabel
      );
      ...
      var displayExpensePercentage = function (expensePercentageNode, index) {
        ...
        if (expensePercentageTotalIncomeArray[index] > 0) {
          expensePercentageNode.textContent =
            rpJSframework.pblFormatNumberUsingSystemLocale(
              expensePercentageTotalIncomeArray[index]
            ) + "%";
        } else {
          expensePercentageNode.textContent = "-";
        }
      ...
      // if no element with this class was found in the DOM
        ...
        console.log(
          "The user did not submit any expenses yet, or there are no nodes, in the DOM,  with the attribute class=%s yet.\n" +
            "Can't display expense percentage out of total income label.",
          prvDOMstrings.expensePercentageLabel
        );
      }
    },

    // Public method for deleting a submitted transaction from the income or expense column based on its type
    // Parameters:
    //   - transId (String): Id value of the 4th ancestor of the event target DOM element.
    ...
    pblClearTransFormFields: function () {
      ...
      transFormDescAndValueFieldsNl = document.querySelectorAll(
        prvDOMstrings.inputDescription + "," + prvDOMstrings.inputValue
      );

      // transform the descAndValueNodeList NodeList into an array
      transFormDescAndValueFieldsArray = Array.prototype.slice.call(
        transFormDescAndValueFieldsNl
      );
      ...
      transFormDescAndValueFieldsArray.forEach(function (
        currentElem,
        currentElemIndex,
        array
      ) {
    ...
    pblColorCodeTransInputFields: function () {
      ...
      rpJSframework.pblNodeListForEach(
        fields,
        function (currentElem, currentElemIndex) {
          currentElem.classList.toggle(
            prvDOMstrings.expenseTransInputFieldsColor
          );
        }
      );

      document
        .querySelector(prvDOMstrings.inputButton)
        .classList.toggle(prvDOMstrings.expenseTransSubmitBtnColor);
    },

    // Public method for displaying on the UI the total income, total expenses, budget, and
    // how much of the total income the expenses represent in percentages
    // Parameters:
    //   - budgetObj (Object): Object containing budget details.
    pblDisplayTransactionOverview: function (budgetObj) {
      // select the budget label on the UI
      document.querySelector(prvDOMstrings.budgetLabel).textContent =
        rpJSframework.pblFormatNumberUsingSystemLocale(budgetObj.budget);

      // select the green background total income label on the UI and display it with two decimals
      document.querySelector(prvDOMstrings.incomeLabel).textContent =
        rpJSframework.pblFormatNumberUsingSystemLocale(budgetObj.totalIncome);

      // select the red background total expenses label on the UI
      document.querySelector(prvDOMstrings.expenseLabel).textContent =
        rpJSframework.pblFormatNumberUsingSystemLocale(budgetObj.totalExpenses);

      // select the expenses percent label to the right of the total expenses label on the UI
      var expensePercentTotalIncElem = document.querySelector(
        prvDOMstrings.expPercentTotalInc
      );
      ...
      if (budgetObj.expensesPercentageIncome > 0) {
        expensePercentTotalIncElem.textContent =
          rpJSframework.pblFormatNumberUsingSystemLocale(
            budgetObj.expensesPercentageIncome
          ) + "%";
      }
    ...
    pblConstructPopupAndRegisterItsEvents: function (text, type, height) {
      var button,
        ...
        popupColor,
        popupNode,
        popupContainerOpeningDivsStr,
        popupContainerClosingDivsStr;

      popupContainerOpeningDivsStr =
        '<div id="alertBox"><div id="alertBox_container"><div id="alertBox_titlebar"><span>myExpenses</span><input id="alert_box_x_button" type="button" value="X"></div><div id="alertBox_text">';
      ...
      if (type === "err") {
        button =
          '<div id="' +
          prvDOMstrings.popupButtonDiv +
          '"><input id="' +
          prvDOMstrings.errorPopupOKbutton +
          '" class="button" type="button" value="OK" alt="OK"></div>';
      ...
      document
        .querySelector(prvDOMstrings.transactionContainer)
        .insertAdjacentHTML("beforeend", popupNode);

      if (height !== undefined) {
        document.getElementById(prvDOMstrings.popupMainDiv).style.height =
          height;
      }
      document.getElementById(
        prvDOMstrings.popupContainer
      ).style.backgroundColor = popupColor;

      // hide the custom popup when it's X button is clicked
      document
        .getElementById(prvDOMstrings.errorPopupXbutton)
        .addEventListener("click", function () {
          document.getElementById(prvDOMstrings.popupMainDiv).style.visibility =
            "hidden";
        });
