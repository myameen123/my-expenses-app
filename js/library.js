var rpJSframework = (function () {
  ...
  var prvGetIndexOfSubstrThatMatchesRegex = function (
    numberAsString,
    regexPattern
  ) {
  ...
  var prvGetThousandSeparatorSystemLocale = function () {
    ...
    var thousandSeparatorIndex = prvGetIndexOfSubstrThatMatchesRegex(
      numberStr,
      regexPattern
    );
  ...
  // Return a string representation of a number, formatted with a symbol for the thousands separator.
  // Parameters:
  //   - num (Number): The number to format.
  // Returns: A string with the formatted number
  var prvFormatNumIntegerPart = function (num) {
    var formatedNumerIntegerPart,
      thousandSeparator,
    ...
    if (numIntegerPart.toString().length > 3) {
      ...
      if (!thousandSeparator) {
        console.info(
          'Could not get the system locale for the number thousand separator. Falling on default separator ","'
        );
        thousandSeparator = ",";
      }
      ...
      for (var i = thousandSeparatorPositions.length - 1; i > 0; i--)
        formatedNumerIntegerPart =
          formatedNumerIntegerPart +
          thousandSeparator +
          numIntegerPartString.substring(
            thousandSeparatorPositions[i],
            thousandSeparatorPositions[i - 1]
          );
      ...
      formatedNumerIntegerPart =
        ...
        thousandSeparator +
        numIntegerPartString.substring(
          thousandSeparatorPositions[0],
          numIntegerPartString.length
        );
    }
  ...
  // Parameters:
  //   - num (Number): The number to extract decimals from.
  // Returns: A string representing the last two decimals.
  var prvReturnTheFirstTwoDecimalsOfNum = function (num) {
    ...
    if (indexOfDecimalInNum !== -1) {
      decimalsOfNumStr = numToString.substring(
        indexOfDecimalInNum + 1,
        numToString.length
      );
      ...
      switch (decimalsOfNumCount) {
        ...
        default:
          inputNumDecimals = numToString.substring(
            indexOfDecimalInNum + 1,
            indexOfDecimalInNum + 3
          );
      }
    }
  ...
  return {
    ...
    // Parameters:
    //   - list (NodeList): The NodeList to iterate over.
    //   - callback (function): The callback function to apply to each element in the NodeList.
    // Returns: None
    ...
    // Parameters:
    //   - number (Number): The number on which to count the decimals.
    // Returns: The number's decimal count or 0 if the number doesn't have decimals
    pblNumberDecimalCount: function (number) {
      var numberToStr = number.toString();
      var numberDecimalChrIndex = numberToStr.indexOf(
        prvGetSystemLocaleSettingDecimalChr()
      );

      if (numberDecimalChrIndex !== -1) {
        return numberToStr.substring(
          numberDecimalChrIndex + 1,
          numberToStr.length
        ).length;
      } else {
        return 0;
    ...
    //   - lastDecimalPosition (Number): The position of the last decimal to keep.
    // Returns: A floating point number with N decimals or 2 by default
    ...
    pblFormatNumberUsingSystemLocale: function (num) {
      ...
      formatedNumber =
        formatedNumberIntegerPart +
        prvGetSystemLocaleSettingDecimalChr() +
        firstTwoDecimalsOfNum;
