...
var controller = (function (budgetCtrl, UIctrl) {
  ...
  var prvSetupEventListeners = function (event) {
    ...
    document
      .querySelector(nodeClassAndIdValues.transactionContainer)
      .addEventListener("click", function (event) {
        ...
        if (
          event.target.id !== nodeClassAndIdValues.errorPopupOKbutton &&
    ...
    document
      .querySelector(nodeClassAndIdValues.inputButton)
      .addEventListener("click", prvAddItem, false);

    // register Enter keypress event for the global object. Used only for the ENTER key
    document.getElementById("enterSum").addEventListener(
      "keypress",
      function (event) {
    ...
    document.querySelector(nodeClassAndIdValues.dateLabel).textContent =
      dateObj.currentMonthName + " " + dateObj.currentYear;
    ...
    document
      .querySelector(nodeClassAndIdValues.inputType)
      .addEventListener("change", uiController.pblColorCodeTransInputFields);
  };

  // Calculate the budget based on the last submitted or selected transaction and display it on the UI.
  // Parameters: None.
  // Returns: None.
  var prvUpdateBudget = function () {
    ...
    budgetIncExpExppercentage =
      budgetController.pblGetBudgetExpIncExpPercentage();
  ...
  // Update the percentage value in the red box next to a submitted expense transaction.
  // Parameters: None.
  // Returns: None.
  var prvUpdatePercentageSubmitedExpenses = function () {
    ...
    expensePercentageOutOfTotalIncomeArray =
      budgetController.pblGetPercentageOfExpenseOutOfTotalIncomeForEachExp();

    // for each submited expense in the UI update the expense percentage
    uiController.pbldisplayPercentageOfExpenseOutOfTotalIncomeForAllExpenses(
      expensePercentageOutOfTotalIncomeArray
    );
  };

  // Validate the user input for a transaction.
  // Parameters:
  //   - userInput (Object): User input for the transaction.
  // Returns: Boolean indicating whether the transaction input is valid or not.
  ...
  // entering the new transaction under the income or expense column on the UI,
  // adjusting the budget according to the user's transaction.
  // Parameters: None.
  // Returns: None.
  var prvAddItem = function () {
    ...
    if (prvIsSubmitedTransactonValid(userInput)) {
      ...
      newTransaction = budgetController.pblAddItem(
        userInput.type,
      ...
      if (!popupNode) {
        ...
        uiController.pblConstructPopupAndRegisterItsEvents(
          popupText,
          "err",
          "22%"
        );
        ...
        popupNode = document.getElementById(
          domNodeClassAndIdValues.popupMainDiv
        );
      ...
      document
        .getElementById(domNodeClassAndIdValues.errorPopupOKbutton)
        .focus();
    }
  };

  // Function for deleting a submitted transaction from the income or expense columns.
  // Parameters:
  //   - event (Click event): An event that was fired up from below the .container div tag.
  // Returns: None.
  var prvDeleteItem = function (event) {
    var itemClassDivAncestorOfEventTargetNode,
      strList,
      transactionType,
      transactionIdNumber,
      domClassesIds,
    ...
    itemClassDivAncestorOfEventTargetNode =
    ...
    if (regexIncExpIdValueDOM.test(itemClassDivAncestorOfEventTargetNode.id)) {
      ...
      if (!isNaN(transactionIdNumber)) {
        ...
        uiController.pblDeleteListItem(
          itemClassDivAncestorOfEventTargetNode.id
        );
        ...
        console.error(
          "The value of the id attribute of %O, doesnt match the regex %s." +
  ...
  return {
    // Public method to initialize variables and events.
    // Parameters: None.
    // Returns: None
